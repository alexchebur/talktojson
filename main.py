...
KEYWORDS_PROMPT = """
Извлеки 10 семантически ключевых терминов из текста, включая:
- Названия законов, постановлений (ГК РФ, КоАП и т.д.).
- Конкретные понятия ("неустойк", "поставк", "теплоснабжен").
Формат: список терминов в нижнем регистре, разделенных запятыми, без окончаний из гласных букв (стемминг).
Запрещены: дубликаты, цифры, слова менее чем из четырех букв, предлоги, союзы, частица "не"
"""
...
class DocumentAnalyzer:
    ...
    def _generate_keywords_from_text(self, text: str) -> List[str]:
        ...
        try:
            messages = [
                {"role": "system", "content": "Ты - эксперт по юридической терминологии. Определи существо спора, подготовь короткое саммари с цитированием правовых актов и составь список из 15 наиболее семантически значимых ключевых слов (один синоним на каждое слово) по этому саммари. Слова приводи без окончаний, если окончания состоят из гласных. ЗАПРЕЩЕНО: 1. Извлекать предлоги. 2. Извлекать цифры. 3. Извлекать слова длиной менее четырех букв. 4. Извлекать общеупотребительные юридические термины: норма, правительство, кодекс, закон, иск, постановление."},
            ...
            with st.sidebar:
                ...
                if not chunks:
                    ...
                    for i, chunk in enumerate(chunks[:3], 1):
                        ...
                        st.write(chunk.get('chunk_text', '')[:20000] + "...")
                        st.divider()
